  ...
  <div class="space-y-6">
    ...
    <template v-else-if="budget">
      ...
      <div class="flex items-center gap-4">
        <NuxtLink :to="`/budgets/${budget.id}`" class="btn btn-ghost btn-sm">
          <svg
        ...
        <div>
          <h1 class="text-2xl font-bold">Edit {{ budget.budgetNumber }}</h1>
...
// Fetch budget
const { data: budget, pending } = await useFetch(`/api/budgets/${budgetId}`)
...
watch(
  ...
  newBudget => {
    if (newBudget) {
      form.customerId = (newBudget as any).customerId || ''
      form.title = (newBudget as any).title || ''
      form.description = (newBudget as any).description || ''
      form.notes = (newBudget as any).notes || ''
      form.items =
        (newBudget as any).items?.map((item: any) => ({
          id: item.id,
...
const handleSubmit = async () => {
  ...
  try {
    ...
    await navigateTo(`/budgets/${budgetId}`)
  } catch (err: any) {
    showError(err.data?.message || 'Gagal update budget')
  } finally {
    loading.value = false
  }
}
